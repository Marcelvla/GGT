...
#define glCheckError(s) { GLenum error; if ((error=glGetError())) \
  ...
  (s), __func__,__FILE__,__LINE__, gluErrorString(error)); }
#else                           /* __GNUC__ */
#define glCheckError(s) { GLenum error; if ((error=glGetError())) \
...
InitializePolygonlists(void)
{
    ...
    loadPolygonalObject(polylistTreeLeafs, "leaf.obj", texture_names, 0.27, 0.0, 1.5, 0.0);
